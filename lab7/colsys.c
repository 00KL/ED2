...
void predict(Particle *p) {
    ...
    for(int i = 0; i < N; i++){
        tEvent = time_to_hit(p, particles[i]);
        if(tEvent + t <= limit){
            Event* e = create_event(tEvent + t, p, particles[i]);
            PQ_insert(pq, e);
        }
    }

    //pq somar o tempo atual e o tEvent?????
    tEvent = time_to_hit_vertical_wall(p);
    if(tEvent + t <= limit){
        Event* e = create_event(t, p, NULL);
        PQ_insert(pq, e);
    }

    tEvent = time_to_hit_horizontal_wall(p);
    if(tEvent + t <= limit){
...
void simulate() {
    ...
    //pq não atualizar depois as paticulas?????????
    // for(int i = 0; i < N; i++){
    //     predict(particles[i]);
    // }
